...
export function ScriptureReferenceChip({
  ...
  const href = getLocalGalatiansReferenceHref(reference);
...
function getLocalGalatiansReferenceHref(reference: string) {
  const match = reference.trim().match(/^Galatians\s+(\d+):(\d+)(?:[-–—]\d+)?$/iu);
  if (!match) return null;

  return `/galatians/${match[1]}/#v${match[2]}`;
}
